...
{
    [Fact]
    public async Task Ingests_eight_entities_from_fixture_and_calls_upsert_once()
    {
        ...
        await store.Received(1).UpsertAsync(
            Arg.Is<IList<EntityPoint>>(p => p.Count == 8),
            Arg.Any<CancellationToken>());
        await tracker.Received(1).MarkEntitiesIngestedAsync(1, 8, Arg.Any<CancellationToken>());
    }
}
