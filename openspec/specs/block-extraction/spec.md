...
### Requirement: Block extraction is performed by docling-serve
The system SHALL extract layout-aware blocks from PDFs by delegating to the docling-serve sidecar. There is no runtime selector — `IPdfBlockExtractor` is bound to `DoclingBlockExtractor` unconditionally. Reading order is supplied by docling-serve; PdfPig and the legacy Docstrum / RecursiveXYCut segmenters are no longer used during ingestion.

#### Scenario: IPdfBlockExtractor resolves to DoclingBlockExtractor
- **WHEN** the application's DI container resolves `IPdfBlockExtractor`
- **THEN** the returned instance is `DoclingBlockExtractor`, which delegates to `IDoclingPdfConverter`
