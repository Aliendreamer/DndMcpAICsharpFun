...
**D2 — Replace the 5etools-existence *proxy* with a book-derived `IsRealEntity` predicate (T2), applied to BOTH official and keyless books, do NOT just delete the decline.** The predicate is `structural-signature OR (entity-like name AND substantial, non-tabular body)`, where structural-signature = the existing complete-stat-block / spell / magic-item signatures PLUS a new **subclass-feature-progression** signature (≥2 level-gated grants, e.g. "Starting at Nth level…", "At Nth level…"). The prose-OR branch admits the gated types that have no structural form (Background, Feat, Race, God). It fires in `DeterministicTypeResolver` on the gated-prior + no-5etools-match branch:
