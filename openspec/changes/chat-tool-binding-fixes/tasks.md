...
- [x] 1.1 In `DndMcpAICsharpFun.Tests/Chat/DndChatServiceTests.cs`, add a data-driven test that
...
- [x] 2.1 In `Features/Chat/DndChatService.cs`, add defaults to the optional params (and
...
- [x] 3.1 `build_encounter`: reorder the lambda params so required (`difficulty`, `edition`)
...
- [x] 3.2 `prep_session`: reorder so required (`campaignId`, `theme`, `npcArchetype`) precede
  optional (`difficulty = null`), `toolCt = default` last; body unchanged.
- [x] 3.3 `rate_encounter`: reorder so required (`monsters`, `edition`) precede optional
  (`campaignId = null`, `partyLevels = null`), `toolCt = default` last; body unchanged.

## 4. Omit-key invocation coverage

- [x] 4.1 Add omit-key `InvokeAsync` cases on 1-2 representative tools (`build_encounter`,
...
- [x] 5.1 `dotnet build` clean; the Task 1 regression test now PASSES; full `dotnet test` green
  (including the existing SEC-08 no-userId and per-tool routing tests).
- [x] 5.2 Rebuild the app image (`docker compose up -d --build app`) and run a live chat smoke on
...
- [x] 5.3 Update the change report with results; commit.
