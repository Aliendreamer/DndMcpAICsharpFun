...
- [x] **Step 1: Read both methods via Serena**
...
- [x] **Step 2: Add a helper that computes the canonical slug the same way as `EntityIngestionOrchestrator`**
...
- [x] **Step 3: Update `ExtractEntities` to use `CanonicalSlugFor`**
...
- [x] **Step 4: Update `BookDeletionService.DeleteBookAsync` to use the same slug**
...
- [x] **Step 1: Read `IngestEntitiesAsync` via Serena**

```
mcp__serena__find_symbol("EntityIngestionOrchestrator/IngestEntitiesAsync", include_body=true)
```

- [x] **Step 2: Guard the delete on empty FileHash and flip order to upsert-first**
...
- [x] **Step 1: Read the interface and implementation**
...
- [x] **Step 2: Update interface — remove `blockCount` parameter**
...
- [x] **Step 3: Update implementation to use payload filter delete**
...
- [x] **Step 4: Fix the call site in `BlockIngestionOrchestrator`**
...
- [x] **Step 3: Update all callers of `ToEnvelope` to handle the nullable return**
...
- [x] **Step 1: Read `EmbedAsync` via Serena**

```
mcp__serena__find_symbol("OllamaEmbeddingService/EmbedAsync", include_body=true)
```

- [x] **Step 2: Add null guard after the Ollama call**
...
- [x] **Step 3: Add guard at the top of `search_entities`**
...
- [x] **Step 4: Add guard at the top of `get_entity`**
...
- [x] **Step 1: Read both files via Serena**
...
- [x] **Step 2: Sanitize the bookSlug in the endpoint before passing to the service**
...
- [x] **Step 3: Replace null-forgiving `!` accesses with null checks in `FixTypesAsync`**
...
- [x] **Step 1: Read both middleware files**
...
- [x] **Step 2: Replace `AdminApiKeyMiddleware.InvokeAsync`**
...
- [x] **Step 3: Apply same fix to `McpAuthMiddleware.InvokeAsync`**
...
- [x] **Step 1: Read `ScorePair` via Serena**

```
mcp__serena__find_symbol("CrossEncoderReranker/ScorePair", include_body=true)
```

- [x] **Step 2: Replace the final two lines of `ScorePair`**
...
- [x] **Step 1: Read `Chat.razor` via Serena**

```
mcp__serena__find_symbol("Chat", "DndMcpAICompanion/Components/Pages/Chat.razor", depth=1)
```

- [x] **Step 2: Implement `IAsyncDisposable`, add a `CancellationTokenSource` field**
...
- [x] **Step 3: Replace `CancellationToken.None` with `_cts.Token`**

```csharp
await ChatService.SendAsync(msg, _webSearchEnabled, _cts.Token);
```

- [x] **Step 4: Build companion project**
...
- [x] **Step 1: Read `AppExtensions` via Serena**

```
mcp__serena__find_symbol("AppExtensions", "DndMcpAICompanion/Extensions/AppExtensions.cs", depth=1, include_body=true)
```

- [x] **Step 2: Replace the blocking register with a fire-and-forget dispose with timeout**
...
- [x] **Step 2: Throw on final failure in `StartAsync`**
...
- [x] **Step 3: Mark `SparseSupported` as `volatile`**
...
- [x] **Step 2: Expand the conflict check in `IngestBlocks`**
...
- [x] **Step 3: Expand the conflict check in `IngestEntities`**
...
- [x] **Step 1: Read `RunFullExtractionAsync` via Serena**

```
mcp__serena__find_symbol("RunFullExtractionAsync", include_body=true, max_answer_chars=10000)
```

- [x] **Step 2: Wrap File.Delete calls in try/catch**
...
- [x] **Step 3: Move `MarkEntitiesExtractedAsync` BEFORE `File.Delete` calls**
...
- [x] **Step 2: Broaden the catch in `LoadSchemas`**
...
- [x] **Step 3: Add temp file cleanup to `WriteCheckpointAsync`**
...
- [x] **Step 2: Wrap in environment check**
...
- [x] **Step 1: Read `GetAllAsync` and `GetAllBooks`**
...
- [x] **Step 2: Add `limit` and `offset` parameters to `IIngestionTracker.GetAllAsync` and implementation**
...
- [x] **Step 3: Update the endpoint to accept query params**
...
- [x] **Step 2: Add `CancellationToken ct = default` to `IPdfBlockExtractor.ExtractBlocks`**

```csharp
IEnumerable<PdfBlock> ExtractBlocks(string filePath, CancellationToken ct = default);
```

- [x] **Step 3: Update `DoclingBlockExtractor.ExtractBlocks` to accept and pass the token**
...
- [x] **Step 1: Read the options classes and ServiceCollectionExtensions**
...
- [x] **Step 2: Add `[Required]` to critical string properties**
...
- [x] **Step 3: Chain `.ValidateDataAnnotations().ValidateOnStart()` in ServiceCollectionExtensions**
...
- [x] **Step 2: Return 422 also for NeedsReview warnings**
...
- [x] **Step 1: Read `RateLimitExtensions` and `Register.razor`**
...
- [x] **Step 2: Add a strict registration limiter**
...
- [x] **Step 3: Apply `[EnableRateLimiting("registration")]` or route-level attribute**
