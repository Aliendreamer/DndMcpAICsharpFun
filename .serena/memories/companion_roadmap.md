...
**FILTER-DEGENERATE-TABLES SHIPPED (2026-07-24)** (`2026-07-24-filter-degenerate-tables`, suite 1645/1645, task review APPROVED zero-findings). The degenerate-table follow-up from the corpus re-extraction: `HtmlTableParser.Parse` now drops **D1** (<2-col OR 0-data-row / header-only grids — `columns.Count < 2 || rows.Count < 2`) + **D2** (stat-block ability-line fragments: a ≤2-row grid with ≥3 cells matching the exact case-sensitive `\b(STR|DEX|CON|INT|WIS|CHA)\b\s*\d`) at the SINGLE parse chokepoint — the collector already skips `null`, so NO collector source change; 3 incidentally-single-row collector caption-test fixtures were widened to real tables (no assertion weakened). Live MTF re-extract left **DEFERRED/optional**: MTF is a 5etools-official book → its tables come from `ProjectTables` (5etools), not MinerU, so the filter's payoff is on the MinerU set (homebrew/keyless books + pre-ProjectTables); books are git-crypt on host (no cheap host-side harness) and a full re-extract is ~8.5 h — deterministic unit tests prove D1/D2, so it applies on the next natural re-extraction. (`table-name-from-heading` was ALSO already shipped+archived 2026-07-20 — the collector names caption-less tables from the preceding heading; the memory drift was caught by the new red flag.) Genuinely-remaining table work: the "why so much table noise" corpus breakdown TODO + wiring auto-collected tables to the resolution engine (id alignment) — see [[table_extraction_quality]]. **Also this session — STALE-CHANGE-DIR DRIFT reconciled:** `automatic-decline-recovery` showed active/0-of-6 tasks but was ALREADY fully implemented, live-validated, and archived as `extraction-cross-type-recovery` (the `DeclineRecovery` service + orchestrator post-loop recovery phase); an implementer was dispatched before the drift surfaced. Archived the stale dir (bookkeeping only, no code) as `2026-07-24-automatic-decline-recovery`, and added a dev-flow red flag: **before implementing any resumed/active OpenSpec change, grep `git log --all` for its key service/commit-subjects to confirm the code isn't already shipped under another name.**

**RESOLUTION-FEATURES SHIPPED (2026-07-24)** (`2026-07-24-resolution-features`, suite 1679/1679, whole-branch APPROVE). Three new grounded `CharacterResolutionService` features (Spec A of a two-spec split): **`class resources`** (per-class resource columns — Rages/Ki/Sorcery Points/Sneak Attack/Invocations — read from the projected class table at level, provenance-cited; Fighter → honest `"none"`, missing/ambiguous → needsReview); **`saving throws`** (new static `SavingThrowProficiencies` PHB class→[2 saves] map, proficient from `Classes[0]` only per 5e, `mod + (proficient?PB:0)`, computed/null-provenance); **`spell count`** (DATA-DRIVEN known-vs-prepared — `Spells Known` cell → known/table-cited; else `SpellcastingAbility!=null` → prepared `mod+level` full / `mod+level/2` half; else non-caster → `"no spellcasting"`). Grounding pre-verified against `phb14.json` columns; per-branch + real-Postgres integration tests (seed real 5etools columns). Reconciled a spec-vs-impl gap the opus whole-branch review found (spec claimed known-caster-missing-table→needsReview, undetectable in the data-driven scheme → softened spec to match). **This arc's origin:** the "wire tables to resolution engine" roadmap item had NO orphaned-table gap (overtaken by project-tables-from-5etools + resolve-class-and-subclass), so the user chose to ADD new resolver features instead — and the AC feature (feature 4) was SPLIT OUT because `CharacterSheet` stores `Equipment` as free-text with no structured worn-armor field. Also: `table_extraction_quality` NOISE INVESTIGATION now DONE — all 44% degenerate tables are MPMM/MTF monster stat-block noise (D1 filter covers it); `table-name-from-heading` confirmed already-shipped (memory drift). See [[table_extraction_quality]].

**STRUCTURED-ARMOR-AND-AC SHIPPED (2026-07-24)** (`2026-07-24-structured-armor-and-ac`, suite 1694/1694, whole-branch APPROVE — Spec B, completes the two-spec split). A structured `WornArmor { ArmorName, Shield, MagicBonus }` on `CharacterSheet` (**JSON column → NO EF migration**; the earlier "migration prerequisite" framing was wrong — old snapshots deserialize to an unarmored default), a static `ArmorCatalog` (12 PHB armors → base AC + category, like `SavingThrowProficiencies`), a PURE `armor class` resolver (unarmored `10+Dex` raised to best Unarmored Defense — Barbarian `10+Dex+Con` shield-ok / Monk `10+Dex+Wis` no-shield / multiclass-higher; Light `base+Dex`, Medium `base+min(Dex,2)`, Heavy `base`; +2 shield; +magic; unknown armor → needsReview; computed → null provenance), and capture-only hero-editor controls (armor dropdown + shield checkbox + magic-bonus; the manual `ArmorClass` int stays independent, resolver never touches it). UI VISUALLY VERIFIED live (desktop+mobile, no overflow); post-review cosmetics fixed (shield checkbox `.checkbox-label` inline + UD-breakdown omits superseded base). **BOTH resolution specs (A `resolution-features` + B `structured-armor-and-ac`) now shipped.** **OPS FIX this session:** the app-run command changed — keys (`Admin__ApiKey`/`Mcp__ApiKey`) now live in **git-crypt `Config/appsettings.*.json`**, compose does NO env injection; correct local run = `docker compose -f docker-compose.yml up -d --build app` with NO `ADMIN_API_KEY=` env var (passing it nullifies/fakes the key → startup crash). Stale `operations/running_the_stack` memory + the dev-flow presentational-gate command were both FIXED.

**TWO MINOR ITEMS SHIPPED (2026-07-24):** (1) **Level-up HP floor** (`8d5b8b3`, suite 1695) — `LevelUpPlanner` now `Math.Max(1, …)` on the hit-die-average + CON HP gain (very-low-CON no longer yields ≤0 HP); one of the deferred final-review minors, now closed. (2) **`race-ability-filter`** (archived `2026-07-24-race-ability-filter`, suite 1704/1704, whole-branch APPROVE) — `abilityBonus=str` on `GET /retrieval/entities/list` returns races boosting that ability (fixed OR choosable), query-time in-memory match mirroring `castableByClass`, NO re-index; `RaceAbilityParser` reads the boosted set from the entity's raw `Fields`; real-Qdrant grounding test. This is the ability-bonus half of read-path frontier **Item 2** (race aggregation) — the resistance/immunity half stays deferred (same pattern over `RaceFields.Resist`/`Immune`). See [[read_path_frontier]]. **MoE de-listed** as a lever (user: tried, didn't work — 16GB+ GPU is the only real model unlock). Remaining software levers: retrieval reranking/section-scoping; session-prep v2; ingest more books; markitdown parser spike (still untouched); race resistance/immunity filter.
